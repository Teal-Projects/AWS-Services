...
Sticky Sessions :
* From a classic Load Balancer. Allow you to bind a user's session to a specific EC2 instances.
* It can be also enable for application Load Balancer but the traffic will be sent to a Target Group Level.
* If a load Balancer balance only the traffic to one server for an user, disable the sticky session.
